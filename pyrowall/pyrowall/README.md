# 🔥 PyroWall — Cross-Platform Firewall with Web Dashboard

PyroWall is a sophisticated, cross-platform network firewall with a beautiful web-based
dashboard. It runs on both **Linux** and **Windows**, inspects network packets in real-time,
enforces configurable rules, and exposes a full control panel at `http://localhost:5000`.
...
PyroWall automatically enters **simulation mode** — it generates synthetic traffic
so you can test all dashboard features without needing real packet capture.

You will see this warning:
```
WARNING  pyrowall.engine: Running in SIMULATION MODE
```
...
│   └── pyrowall.db          # Auto-created on first run
├── web/
...
- The dashboard binds to `127.0.0.1` only — it is **never exposed to the network**
- The secret key in `config.yaml` should be changed for any serious deployment
- PyroWall operates in **monitoring + enforcement** mode: Scapy logs packets and
  the OS adapter (iptables/netsh) enforces the rules at the kernel level
