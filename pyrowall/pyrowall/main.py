...
log = logging.getLogger('pyrowall')
...
def check_privileges():
    ...
    if platform.system() == 'Windows':
        ...
        if not ctypes.windll.shell32.IsUserAnAdmin():
            log.error("PyroWall requires Administrator privileges on Windows.")
            log.error("Please right-click and 'Run as Administrator'.")
            sys.exit(1)
    else:
        if os.geteuid() != 0:
            log.error("PyroWall requires root privileges on Linux.")
...
""")
    ...
    try:
        ...
        log.info("Shutting down PyroWall...")
        engine.stop()
        sys.exit(0)
