...
log = logging.getLogger('pyrowall.windows')
...
def apply_rule(rule):
    """Translate a PyroWall rule to netsh advfirewall rule."""
    rule_name = f"PyroWall_{rule['id']}_{rule['name'].replace(' ', '_')}"
...
def block_ip(ip):
    run(f'netsh advfirewall firewall add rule name="PyroWall_Block_{ip}" dir=in action=block remoteip={ip}')
    run(f'netsh advfirewall firewall add rule name="PyroWall_Block_{ip}_out" dir=out action=block remoteip={ip}')


def unblock_ip(ip):
    run(f'netsh advfirewall firewall delete rule name="PyroWall_Block_{ip}"')
    run(f'netsh advfirewall firewall delete rule name="PyroWall_Block_{ip}_out"')


def flush_pyrowall_rules():
    run('netsh advfirewall firewall delete rule name="PyroWall*"')
